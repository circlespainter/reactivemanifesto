...
## <a name="Actor-System"></a>Actor-System
Any system modeled after the [actor model](http://en.wikipedia.org/wiki/Actor_model).
...
Isolation can be defined in terms of decoupling, both in time and space. Decoupling in time means that the sender and receiver can have independent life-cycles—they do not need to be present at the same time for communication to be possible. It is enabled by adding boundaries between the [components](#Component). Decoupling in space (defined as [Location Transparency](#Location-Transparency)) means that the sender and receiver do not have to run in the same process, but wherever the operations division or the runtime itself decides is most efficient—which might change during an application's lifetime. 
...
Strong isolation between components can be built on communication over well-defined [protocols](#Protocol) to help enables loose coupling, leading to systems that are easier to understand, extend, test and evolve.
